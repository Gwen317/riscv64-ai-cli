...
crush run --continue "Follow up on your last response"
  ...
  `,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err := validateRunModeForBuild(useClientServer()); err != nil {
			return err
		}
		...
		ws, cleanup, err := setupLocalWorkspaceWithOptions(cmd, app.Options{Headless: true})
		if err != nil {
			return err
		}
		defer cleanup()
...
func validateRunModeForBuild(clientServer bool) error {
	if isHeadlessBuild() && clientServer {
		return fmt.Errorf("client/server mode is not supported in headless builds; unset CRUSH_CLIENT_SERVER or use a non-headless build")
	}
	return nil
}
...
func runNonInteractive(
	...
	spinner = newRunSpinner(ctx, cancel, hideSpinner, stdinTTY, stdoutTTY, stderrTTY)
