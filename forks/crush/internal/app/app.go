...
func New(ctx context.Context, conn *sql.DB, store *config.ConfigStore) (*App, error) {
	return NewWithOptions(ctx, conn, store, Options{})
}

// NewWithOptions initializes a new application instance with runtime options.
func NewWithOptions(ctx context.Context, conn *sql.DB, store *config.ConfigStore, opts Options) (*App, error) {
	q := db.New(conn)
...
func validateNonInteractiveCapabilities(cfg *config.Config, opts Options) error {
	if opts.Headless && len(cfg.MCP) > 0 {
		return fmt.Errorf("headless mode does not support MCP initialization yet; disable MCP entries in the configuration before using non-interactive headless runs")
	}
	return nil
}
...
func (app *App) RunNonInteractive(ctx context.Context, output io.Writer, prompt, largeModel, smallModel string, hideSpinner bool, continueSessionID string, useLast bool) error {
	...
	if err := validateNonInteractiveCapabilities(app.config.Config(), app.options); err != nil {
		return err
	}
	...
	spinner = newAppSpinner(ctx, cancel, output, hideSpinner, stdinTTY, stdoutTTY, stderrTTY)
	...
	if !app.options.Headless && len(app.config.Config().MCP) > 0 {
